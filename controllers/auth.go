...
func (ac *authController) LoadRoutes(router *gin.RouterGroup) {
    ...
    assertDatabase(ac.db, "auth")

    router.GET("/login", ac.ShowLoginForm)
    router.POST("/login", ac.Login)
    router.GET("/register", ac.ShowRegisterForm)
...
func (ac *authController) ShowLoginForm(ctx *gin.Context) {
    views.Login().Render(ctx, ctx.Writer)
}

func (ac *authController) ShowRegisterForm(ctx *gin.Context) {
...
func (ac *authController) sendVerificationEmail(username, email, code string) {
    ...
    err := SendEmail(email, "Xác thực tài khoản QuizCreator của bạn", htmlBody)
    if err != nil {
...
func (ac *authController) OAuthCallback(ctx *gin.Context) {
    ...
    userInfo, err := getUserInfoFromProvider(ctx.Request.Context(), provider, token.AccessToken)
    if err != nil {
...
func getUserInfoFromProvider(ctx context.Context, provider, accessToken string) (*OAuthUserInfo, error) {
    switch provider {
    case "google":
...
func getGithubUserInfo(ctx context.Context, accessToken string) (*OAuthUserInfo, error) {
    client := &http.Client{}
    ...
    email := result.Email
    if email == "" {
        email, _ = getGithubEmail(ctx, accessToken)
    }
